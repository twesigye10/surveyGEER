...
hex_diameter <- 12000 # 12 km set as default diameter for hexagon
...
if(!is.null(write_outputs_to)){
  ...
  grid_w_ndvi_spatial |> 
    ...
    sf::st_write(hex_file_path_name, append=FALSE, overwrite=T)
...
record_lows_2021 <- yearly_rainfall_hex_1981_2021 |> 
  filter(record_low) |> 
  filter(date=="2021-01-01")

record_highs_2021 <- yearly_rainfall_hex_1981_2021 |> 
  filter(record_high) |> 
  filter(date=="2021-01-01")
...
if(!is.null(write_outputs_to)){
  precip_file_path_name <- paste0(write_outputs_to,
                               "reach_",
                               country_code,
                               "_precip_record_low.shp")
  sf::st_write(obj = record_low_centroids, dsn = precip_file_path_name, append = FALSE)
}
```
...
```{r}
# choose a shapefile/layer for admin boundaries
df_choose_shapefile <- choose.files(multi = FALSE, 
                                    caption = "Choose layer to support atlas", 
                                    filters = matrix(c("Shape files (*.shp)", "*.shp", "Geopackage files (*.gpkg)", "*.gpkg"),
                                                     nrow = 2, ncol = 2, byrow = TRUE), 
                                    index = 1) 

eth_adm0_dissolved <- st_read(df_choose_shapefile) |> 
  summarise() |> 
  mutate(uid=1)

atlas_victim <- eth_adm0_dissolved |> 
  left_join(
  grid_with_ndvi |> 
  mutate(yr_mo = paste0(lubridate::month(date,label=T,abbr = F)," ",lubridate::year(date))) |> 
  distinct(date, yr_mo) |> 
...
atlas_victim|> 
  st_write(
    paste0(write_outputs_to, "reach_", country_code, "_atlas_coverage.gpkg"), 
    append = FALSE, 
    overwrite=T
  )
```
